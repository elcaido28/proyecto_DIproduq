...
function agregar() {
  ...
  td.innerHTML = "<select id='producto_" + contLin + "' name='producto" + contLin + "' value='' onpaste='return false' required>"+"<option>-SELECCIONE-</option>"+"<?php $consulta=mysqli_query($con,"SELECT * ,U.descrip descripU from inventario_mp IP inner join unidad U on U.id_unidad=IP.id_unidad where cantidad !='0'");
  while($row=mysqli_fetch_array($consulta)){ ?>"+"<option value='"+"<?php echo $row['id_inventario_mp']; ?>"+"'>"+"<?php echo $row['codigo']." - ".$row['nombre']." ".$row['descripU']; ?>"+"</option>"+"<?php } ?>"+"</select>";
    ...
    <div class="">
      <div class="page-title">
        <div class="col-md-12 col-sm-12 col-xs-12">
          <div class="x_panel">
            <div class="x_title">
              <h2>Registrar Factura Materia Prima </h2>
              ...
              <form id="form21" class="formulario" action="" method="post" >
                <fieldset class="fieldset_grande">
                  ...
                  <div class="cont_cajas_peque">
                    ...
                    <input type="txt" name="nfactura" value="" placeholder="" required >
                  </div>
                  <div class="cont_cajas_peque">
                    <label class="etiq_caja">Fecha</label>
                    <input type="txt" name="fecha" value="<?php echo date('d-m-Y'); ?>" placeholder="" required readonly>
                  </div>
                  <div class="cont_cajas_peque">
                    <label class="etiq_caja">RUC/CI - Cliente</label>
                    <input type="txt" name="cedula" id="cedula" value="" placeholder="" required onkeypress="return solonumeros(event)" onpaste="return false">
                  </div>
                  <div class="cont_cajas_peque">
                    <label class="etiq_caja">Nombres Cliente</label>
                    <input type="txt" name="cliente" id="cliente" value="" placeholder="" required onkeypress="return sololetras(event)" onpaste="return false">
                  </div>
                  ...
                  <div class="cont_cajas_peque">
                    ...
                    <?php $consulta4=mysqli_query($con,"SELECT * from tipo_pago");
                      ...
                      echo "<option value='".$row4['id_tipo_pago']."'>"; echo $row4['descrip']; echo "</option>"; } ?> </select>
                  </div>
                  <div class="cont_cajas_peque">
                    ...
                    <tr>
                      	<td>
                      		<select name="producto1" id="producto_1" onpaste="return false" required>
                      			...
                      			<?php $consulta=mysqli_query($con,"SELECT *,U.descrip descripU from inventario_mp IP inner join unidad U on U.id_unidad=IP.id_unidad where cantidad !='0'");
                        			while($row=mysqli_fetch_array($consulta)){ ?>
                      			<option value="<?php echo $row['id_inventario_mp']; ?>"><?php echo $row['codigo']." - ".$row['nombre']." / ".$row['descripU']; ?></option><?php } ?>
                    		</select>
                      	</td>
                  ...
                  <div class="cont_cajas_peque2">
                    ...
                    <input type="txt" name="valor" value="" id="valor" required readonly>
                  </div>
                  <div class="cont_cajas_peque2">
                    <label class="etiq_caja">descuento</label>
                    <input type="txt" name="descuento" value="" id="descuento" required onkeypress="return solonumeros(event)" onpaste="return false" onChange="calculo2total(this.value);">
                  </div>
                  <div class="cont_cajas_peque2">
                    <label class="etiq_caja">Sub total</label>
                    <input type="txt" name="subtotal" value="" id="subtotal" required readonly>
                  </div>
                  <div class="cont_cajas_peque2">
                    <label class="etiq_caja">iva</label>
                    <input type="txt" name="iva" value="12" id="iva" required readonly>
                  </div>
                  <div class="cont_cajas_peque2">
                    <label class="etiq_caja">TOTAL P.</label>
                    <input type="txt" name="totalp" value="" id="totalp" required readonly>
                  </div>

                  <div class="conten_botom_formu">
                    <input  type="submit" name="enviar" value="Registrar" class="acao">
                    <a href="ingreso_factura_product.php" class="cancelar">Cancelar</a>
                  </div>
                </fieldset>
              </form>
            </div>
...
<script type="text/javascript">
    ...
    function calcu(c_id){
      ...
      var t = document.getElementById(id_preciou).value;
      if (preciou1!=t) {
        alert();
      }
    ...
    var url1='guardar_factura_mp.php?cont='+con+'';
