...
$consulta=mysqli_query($con,"SELECT * from detalle_factu_produc where id_factura_productos='$periodo'");
...
$salida .="<tr><td><select id='producto_".$contLin."' name='producto".$contLin."'  onpaste='return false' required>"."<option>-SELECCIONE-</option>";
 $consult=mysqli_query($con,"SELECT * ,U.descrip descripU from inventario_produc IP inner join unidad U on U.id_unidad=IP.id_unidad where cantidad !='0'");
while($rows=mysqli_fetch_array($consult)){ if($rows['id_inventario_produc']==$row['id_inventario_produc']){$sel="selected='selected'";}else{$sel="";}
$salida .="<option ".$sel." value='".$rows['id_inventario_produc']."'>".$rows['nombre']." - ".$rows['cantidad_presenta']." ".$rows['descripU']." </option>";
 }
